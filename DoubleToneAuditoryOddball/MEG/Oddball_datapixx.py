# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 16:55:47 2023
...
Trigger
-------
- DATAPixx AnalogOut 3 (Standard)
- DATAPixx AnalogOut 4 (Target)
...
- 4 analog channels 
  ...
  - 2 BNCs for audio triggers
    - Standard: BNC (Analog3) to MEG Trigger Interface (STI001) 
    - Target: BNC (Analog4) to MEG Trigger Interface (STI002)
...
# Target and standard event values aren't defined here. Their value depend on
# the wiring between Datapixx AnalougOut and MEG Trigger Interface.
event_values = {
...
# 2: standard trial
# 3: target trial
channel_mapping = {'standard': [0,1,2],
                   'target': [0,1,3],
                   }
...
for trial, trialtype in enumerate(playmatrix):
    ...
    if trialtype == trialtypes['standard']:
        ...
        channel = channel_mapping['standard']
        print('\nStandard trial')
        print('--------------')
    ...
    #-------
        ...
        channel = channel_mapping['target']
        print('\nTarget trial')
        print('------------')
    ...
    analog_signal = np.stack((audio,audio,trigger),axis=0)
    
    numBufferFrames = len(audio)
    maxScheduleFrames = numBufferFrames
    StimDur = numBufferFrames / fs
    dp.DPxWriteDacBuffer(analog_signal, AnalogbufferAddress, channel)
    dp.DPxWriteRegCache()
    
    # Start playback
    #--------------------------------------------------------------------------
    dp.DPxSetDacSchedule(0, # Onset delay
                         fs, # sampling rate
                         "Hz", # rateUnits (str) – This can have three values: “Hz” for samples/seconds, “video” for samples/video frame or “nano” for seconds/samples
                         maxScheduleFrames, 
                         channel, # If provided, it needs to be a list of size nChans.
                         AnalogbufferAddress, 
                         numBufferFrames)
    dp.DPxStartDacSched()    
